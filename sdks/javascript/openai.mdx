...
<Tip>
  **Full examples**: [customer-support-agent](https://github.com/getfoil/foil-examples/tree/main/customer-support-agent) — same agent in two flavors: [native SDK](https://github.com/getfoil/foil-examples/blob/main/customer-support-agent/agent-native-sdk.js) and [OTEL auto-instrumentation](https://github.com/getfoil/foil-examples/blob/main/customer-support-agent/agent-otel.js)
</Tip>

## Quick Start (Recommended)

The easiest way to trace OpenAI calls - just initialize Foil and your calls are automatically traced:
...
For more control, use the `wrapOpenAI` method:
...
<Tabs>
  <Tab title="OpenTelemetry">
    ...
    // ↑ Streaming call automatically traced with TTFT
    ```
  </Tab>
  <Tab title="Manual Wrapper">
    ```javascript
    await tracer.trace(async (ctx) => {
      const wrappedOpenAI = tracer.wrapOpenAI(openai, { context: ctx });
