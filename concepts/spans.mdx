...
```javascript
// Recommended: use ctx.llmCall() — automatically creates and closes the span
...
For advanced use cases (custom token reporting, manual timing), use `ctx.startSpan()`:

```javascript
const span = await ctx.startSpan(SpanKind.LLM, 'gpt-4o', { input: messages });
...
Span inputs and outputs can contain **content blocks** — a mix of text and media references. Use the `content()` helper to create them:

```javascript
import { content, ContentBlock } from '@getfoil/foil-js';

const span = await ctx.startSpan(SpanKind.LLM, 'gpt-4o', {
  input: content(
    'Analyze this document:',
    ContentBlock.media(upload.mediaId, { category: 'document' }),
    'Summarize the key findings.'
  )
});
```

See [Multimodal & Media](/features/multimodal) for full details on uploading files, content block types, and processing.
