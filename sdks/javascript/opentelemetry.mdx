...
| `endpoint` | string | `https://api.foil.dev/api/otlp/v1/traces` | OTLP endpoint |
...
```javascript
const { attachImage, attachCode, attachText, addAttachment } = require('@foil/foil-js/otel');
...
tracer.startActiveSpan('process-image', async (span) => {
  ...
  attachImage(span, imageBase64Data, 'input');

  // Attach code
  attachCode(span, 'console.log("hello")', 'javascript', 'output');

  // Attach text
  attachText(span, 'Additional context here', 'input');

  // Generic attachment
  addAttachment(span, 'text', 'Some text', 'input');
...
|------|-------------|-----------------|
| `image` | Base64-encoded image data | `attachImage(span, base64, role)` |
| `code` | Code snippet with language | `attachCode(span, code, language, role)` |
| `text` | Plain text content | `attachText(span, text, role)` |

The `role` parameter can be `'input'` or `'output'` to indicate the attachment's position in the span.
